<?php
$conn = new mysqli('127.0.0.1:3306', 'u711328531_geekahertz', 'Mehul1206@', 'u711328531_geekahertz') or die(mysqli_errno());
    ...
    if($stmt) {
        ...
        if($data >= 3) {
            ...
            for($i = 1; $i <= count($arrs); $i++) {
                $ords = $i - 1;
                $dateNow = date('Y-m-d');
                mysqli_query($conn, "INSERT INTO `request`(`user_id`, `hospital_id`, `status`, `request_sent_on`, `latitude`, `longitude`, `dispatch_status`) VALUES('$userId', '$arrs[$ords]', 'pending', '$dateNow', '$userLatitude', '$userLongitude', 'not_dispatched')");
            }
            echo 'sent_pending';
        }
    }
